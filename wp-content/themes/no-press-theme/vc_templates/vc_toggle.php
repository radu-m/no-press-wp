...
<div <?php echo isset($el_id) && !empty($el_id) ? "id='" . esc_attr($el_id) . "'" : ''; ?>
    class="no-press-vc-toggle <?php echo esc_attr($css_class); ?>">
    <div
        class="no-press-toggle-title vc_toggle_title"><?php echo apply_filters('wpb_toggle_heading', '<h4>' . esc_html($title) . '</h4>', array(
            'title' => $title,
            'open' => $open,
        )); ?><i class="vc_toggle_icon"></i></div>
    <div
        class="no-press-toggle-content vc_toggle_content"><?php echo wpb_js_remove_wpautop(apply_filters('the_content', $content), true); ?></div>
</div>
