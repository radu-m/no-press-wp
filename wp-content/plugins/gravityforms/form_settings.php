...
class GFFormSettings
{
    ...
    public static function form_settings_ui()
    {
        ...
        <div class="gform_panel gform_panel_form_settings" id="form_settings">
            ...
            <form action="" method="post" id="gform_form_settings">
                ...
                <input type="button" id="gform_save_settings" name="gform_save_settings"
                       value="<?php _e('Update Form Settings', 'gravityforms'); ?>" class="button-primary gfbutton"
    ...
    public static function confirmations_list_page($form_id)
    {
        ...
        <h3><span><i class="fa fa-envelope-o"></i> <?php _e('Confirmations', 'gravityforms') ?>
                <a id="add-new-confirmation" class="add-new-h2"
                   href="<?php echo esc_url($add_new_url) ?>"><?php _e('Add New', 'gravityforms') ?></a></span>
        </h3>
    ...
    public static function confirmations_edit_page($form_id, $confirmation_id)
    {
            ...
            function MaybeAddSaveMergeTags(mergeTags, elementId, hideAllFields, excludeFieldTypes, isPrepop, option) {
                ...
                if (event == 'form_saved' || event == 'form_save_email_sent') {
                    mergeTags["other"].tags.push({
                        tag: '{save_link}',
                        label: '<?php _e( 'Save &amp; Continue Link', 'gravityforms' ) ?>'
                    });
                    mergeTags["other"].tags.push({
                        tag: '{save_token}',
                        label: '<?php _e( 'Save &amp; Continue Token', 'gravityforms' ) ?>'
                    });
                }
                ...
                if (event == 'form_saved') {
                    mergeTags["other"].tags.push({
                        ...
                        label: '<?php _e( 'Save &amp; Continue Email Input', 'gravityforms' ) ?>'
                    });
                }
        ...
        <div id="confirmation-editor">
            ...
            <form id="confirmation_edit_form" method="post">
                ...
                <input type="hidden" id="confirmation_id" name="confirmation_id"
                       value="<?php echo $confirmation_id; ?>"/>
                <input type="hidden" id="form_id" name="form_id" value="<?php echo $form_id; ?>"/>
                <input type="hidden" id="is_default" name="is_default"
                       value="<?php echo rgget('isDefault', $confirmation) ?>"/>
                <input type="hidden" id="conditional_logic" name="conditional_logic"
                       value="<?php echo htmlentities(json_encode(rgget('conditionalLogic', $confirmation))); ?>"/>

                <p class="submit">
                    <input type="submit" name="save" value="<?php _e('Save Confirmation', 'gravityforms'); ?>"
    ...
    public static function get_confirmation_ui_settings($confirmation)
    {
        ...
        <tr <?php echo $is_default ? 'style="display:none;"' : ''; ?> class="<?php echo $class; ?>">
            ...
            <td>
                <input type="text" id="form_confirmation_name" name="form_confirmation_name"
                       value="<?php echo rgar($confirmation, 'name'); ?>"/>
            </td>
        ...
        <tr>
            ...
            <td>
                <input type="radio" id="form_confirmation_show_message"
                       name="form_confirmation" <?php checked('message', $confirmation_type); ?> value="message"
                ...
                &nbsp;&nbsp;
                <input type="radio" id="form_confirmation_show_page"
                       name="form_confirmation" <?php checked('page', $confirmation_type); ?> value="page"
                       onclick="ToggleConfirmation();"/>
                ...
                &nbsp;&nbsp;
                <input type="radio" id="form_confirmation_redirect"
                       name="form_confirmation" <?php checked('redirect', $confirmation_type); ?> value="redirect"
                       onclick="ToggleConfirmation();"/>
        ...
        <tr id="form_confirmation_message_container" <?php echo $confirmation_type != 'message' ? 'style="display:none;"' : ''; ?> >
            ...
            <td>
                ...
                if (GFCommon::is_wp_version('3.3')) {
                    ...
                    ?>
                    <textarea name="form_confirmation_message" id="form_confirmation_message"
                              class="fieldwidth-1 fieldheight-1"><?php echo esc_html($confirmation['message']) ?></textarea><?php
                }
                ?>
                <div style="margin-top:5px;">
                    <input type="checkbox" id="form_disable_autoformatting" name="form_disable_autoformatting"
                           value="1" <?php echo empty($confirmation['disableAutoformat']) ? '' : "checked='checked'" ?> />
                    <label
                        for="form_disable_autoformatting"><?php _e('Disable Auto-formatting', 'gravityforms') ?><?php gform_tooltip('form_confirmation_autoformat') ?></label>
                </div>
            </td>
        ...
        <tr class="form_confirmation_page_container" <?php echo $confirmation_type != 'page' ? 'style="display:none;"' : '' ?>
        ...
        <tr class="form_confirmation_page_container" <?php echo $confirmation_type != 'page' ? 'style="display:none;"' : '' ?>
            ...
            <td>
                <input type="checkbox" id="form_page_use_querystring"
                       name="form_page_use_querystring" <?php echo empty($confirmation['queryString']) ? '' : "checked='checked'" ?>
                       onclick="TogglePageQueryString()"/>
                <label
                    for="form_page_use_querystring"><?php _e('Pass Field Data Via Query String', 'gravityforms') ?></label>

                <div
                    id="form_page_querystring_container" <?php echo empty($confirmation['queryString']) ? 'style="display:none;"' : ''; ?> >
                    <?php
                    $query_string = rgget('queryString', $confirmation);
                    ?>
                    <textarea name="form_page_querystring" id="form_page_querystring"
                              class="merge-tag-support mt-position-right mt-hide_all_fields mt-option-url"
                              style="width:98%; height:100px;"><?php echo esc_html($query_string); ?></textarea><br/>

                    <div
                        class="instruction"><?php _e('Sample: phone={Phone:1}&email={Email:2}', 'gravityforms'); ?></div>
                </div>
            </td>
            <?php echo $subsetting_close; ?>
        ...
        <tr class="form_confirmation_redirect_container <?php echo $class; ?>" <?php echo $confirmation_type != 'redirect' ? 'style="display:none;"' : '' ?> >
            ...
            <td>
                <input type="text" id="form_confirmation_url" name="form_confirmation_url"
                       value="<?php echo esc_attr(rgget('url', $confirmation)); ?>" style="width:98%;"/>
            </td>
            <?php echo $subsetting_close; ?>
        </tr> <!-- / confirmation url -->
        ...
        <tr class="form_confirmation_redirect_container" <?php echo $confirmation_type != 'redirect' ? 'style="display:none;"' : '' ?> >
            ...
            <th><?php _e('Redirect Query String', 'gravityforms'); ?><?php gform_tooltip('form_redirect_querystring') ?></th>
            <td>
                <input type="checkbox" id="form_redirect_use_querystring"
                       name="form_redirect_use_querystring" <?php echo empty($confirmation['queryString']) ? '' : "checked='checked'" ?>
                       onclick="ToggleQueryString()"/>
                <label
                    for="form_redirect_use_querystring"><?php _e('Pass Field Data Via Query String', 'gravityforms') ?></label>

                <div
                    id="form_redirect_querystring_container" <?php echo empty($confirmation['queryString']) ? 'style="display:none;"' : ''; ?> >

                    <?php
                    $query_string = rgget('queryString', $confirmation);
                    ?>

                    <textarea name="form_redirect_querystring" id="form_redirect_querystring"
                              class="merge-tag-support mt-position-right mt-hide_all_fields mt-option-url"
                              style="width:98%; height:100px;"><?php echo esc_html($query_string); ?></textarea><br/>
        ...
        <tr <?php echo rgget('isDefault', $confirmation) ? 'style="display:none;"' : ''; ?> >
            ...
            <td>
                <input type="checkbox" id="confirmation_conditional_logic" name="confirmation_conditional_logic"
...
public static function page_header($title = '')
{
    ...
    <div class="wrap gforms_edit_form <?php echo GFCommon::get_browser_class() ?>">
        <h2 class="gf_admin_page_title">
            <span><?php echo $title ?></span><span class="gf_admin_page_subtitle"><span
