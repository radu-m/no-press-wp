...
<div class="wrap">
    ...
    <p class="error fade"
       style="padding:10px;"><?php _e('WPML compatibility packages will soon be obsolete and removed. Please use other means to make your site multilingual-ready.', 'sitepress') ?></p>
                ...
                <table id="icl_packages_<?php echo $package_type ?>" class="widefat">
                    ...
                    <?php foreach ($packages as $package_id => $package): ?>
                        ...
                        <tr <?php if ($incr % 2 == 0): ?>class="alternate"<?php endif ?>>
                            <td>
                                <input type="hidden"
                                       name="icl_packages[<?php echo $package_type ?>][<?php echo $package_id ?>]"
                                       value="0"/>
                                <input type="checkbox"
                                       name="icl_packages[<?php echo $package_type ?>][<?php echo $package_id ?>]"
                                       value="1"<?php echo $checked; ?> />
                            </td>
                            <td><?php if ($package['URI']) echo '<a href="' . $package['URI'] . '">';
                                echo $package['Name'];
                                if ($package['URI']) echo '</a>'; ?></td>
                            <td><?php echo $package['Version']; ?></td>
                            <td><?php echo $package['Description']; ?></td>
                            <?php if ($package_type == 'themes'): ?>
                                <td><?php echo $package['Theme'];
                                    if ($package['ThemeVersion']) echo ' (' . $package['ThemeVersion'] . ')'; ?></td>
                            <?php endif; ?>
                            <?php if ($package_type == 'plugins'): ?>
                                <td><?php echo $package['Plugin'];
                                    if ($package['PluginVersion']) echo ' (' . $package['PluginVersion'] . ')'; ?></td>
                            <?php endif; ?>
                            <td><?php if ($package['AuthorURI']) echo '<a href="' . $package['AuthorURI'] . '">';
                                echo $package['Author'];
                                if ($package['AuthorURI']) echo '</a>'; ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                    <tfoot>
