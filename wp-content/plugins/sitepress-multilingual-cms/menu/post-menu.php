...
if ($icl_lang_duplicate_of): ?>
    <div class="icl_cyan_box"><?php
        ...
        ?>
        <p><input id="icl_translate_independent" class="button-secondary" type="button"
                  value="<?php _e('Translate independently', 'sitepress') ?>"/></p>
...
<?php endif; ?>
    ...
    <div id="icl_document_language_dropdown" class="icl_box_paragraph">
        <p>
            <label
                for="icl_post_language"><strong><?php printf(__('Language of this %s', 'sitepress'), $post_type_label); ?></strong></label>
        </p>
        ...
        if (!$is_original && ($selected_language != $source_language || (isset($_GET['lang']) && $_GET['lang'] != $source_language)) && 'all' != $this->get_current_language()) {
            ...
            <div id="icl_translation_of_panel" class="icl_box_paragraph">
                    ...
                    if (!$is_original || !$source_language || $source_language == $selected_language) {
                        if ($trid) {
                            ...
                            if (isset($src_language_title) && !isset($_GET['icl_ajx'])) {
                                ?>
                                <option value="<?php echo $source_element_id ?>"
                                        selected="selected"><?php echo esc_html($src_language_title); ?>&nbsp;</option>
                                <?php
                            }
                        } else {
                            ?>
                        ...
                        if (icl_is_language_active($selected_language)) {
                            foreach ($untranslated as $translation_of_id => $translation_of_title) {
                                ?>
                                <option
                                    value="<?php echo $translation_of_id ?>"><?php echo esc_html($translation_of_title); ?>
                                    &nbsp;</option>
                                <?php
                            }
                        }
                ...
                if ($disabled && !empty($source_element_id)) {
                    ?>
                    <input type="hidden" name="icl_translation_of" id="icl_translation_of_hidden"
                           value="<?php echo $source_element_id; ?>">
                    <?php
                }
                ?>
            </div>
...
<?php if (isset($_GET['action']) && $_GET['action'] == 'edit' && $trid): ?>
    ...
    <div id="icl_translate_options">
            ...
            <table width="100%" id="icl_untranslated_table" class="icl_translations_table">
                    ...
                    <tr <?php if ($oddev < 0): ?>class="icl_odd_row"<?php endif; ?>>
                        <?php if (!isset($translations[$lang['code']]->element_id)): ?>
                            ...
                            <td align="right">
                                ...
                                ?>
                                <input<?php echo $ddisabled ?> type="checkbox" name="icl_dupes[]"
                ...
                <tr>
                    <td colspan="3" align="right">
                        <input id="icl_make_duplicates" type="button" class="button-secondary"
                               value="<?php echo esc_attr('Duplicate', 'sitepress') ?>" disabled="disabled"
                               style="display:none;"/>
        ...
        <?php if ($translations_found > 0): ?>
            <?php if (!empty($iclTranslationManagement)) {
                $dupes = $iclTranslationManagement->get_duplicates($post->ID);
            } ?>
            <div class="icl_box_paragraph">
                ...
                <table width="100%" class="icl_translations_table" id="icl_translations_table"
                       <?php if (empty($this->settings['show_translations_flag'])): ?>style="display:none;"<?php endif; ?>>
                    <?php $oddev = 1; ?>
                    <?php foreach ($active_languages as $lang): if ($selected_language == $lang['code']) continue; ?>
                        <tr <?php if ($oddev < 0): ?>class="icl_odd_row"<?php endif; ?>>
                            <?php if (isset($translations[$lang['code']]->element_id)): ?>
