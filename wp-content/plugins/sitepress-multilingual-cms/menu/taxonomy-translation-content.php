...
<div id="wpml_tt_taxonomy_translation_wrap">
        ...
        <label>
            ...
            <select id="icl_tt_tax_switch">
                <?php foreach ($wp_taxonomies as $tax_key => $tax): if (!$sitepress->is_translated_taxonomy($tax_key))
                    continue; ?>
                    <option value="<?php echo $tax_key ?>"
                            <?php if ($this->taxonomy == $tax_key): ?>selected="selected"<?php endif; ?>><?php echo $tax->labels->name ?></option>
                <?php endforeach; ?>
            </select>
        </label>
    <?php endif; ?>
    ...
    <div class="icl_tt_tools">
        <form id="wpml_tt_filters">
            ...
            <select name="status">
                <option value="<?php echo WPML_TT_TAXONOMIES_NOT_TRANSLATED ?>"
                        <?php if ($this->status == WPML_TT_TAXONOMIES_NOT_TRANSLATED):
            ...
            &nbsp;
            <input type="text" name="search" placeholder="<?php esc_attr_e('search', 'sitepress') ?>"
                   value="<?php if (isset($this->search))
                       echo esc_attr($this->search) ?>"/>
            <?php if (isset($this->search)): ?>
                <img id="wpml_tt_clear_search" src="<?php echo ICL_PLUGIN_URL ?>/res/img/close2.png" width="10"
                     height="10" alt="<?php esc_attr_e('clear', 'sitepress') ?>"
                     title="<?php esc_attr_e('clear search', 'sitepress') ?>"/>
            <?php endif; ?>
            &nbsp;
            ...
            <img src="<?php echo ICL_PLUGIN_URL . '/res/img/ajax-loader.gif' ?>" alt="loading" height="16" width="16"
                 class="wpml_tt_spinner"/>
    ...
    <div class="icl_tt_main_top">
        ...
        <table class="wp-list-table widefat fixed">
                    ...
                    <tr>
                        <?php foreach ($this->selected_languages as $language): ?>
                            <td>
                                ...
                                <a class="icl_tt_term_name<?php echo $class ?>" href="#"
                                   onclick="WPML_Translate_taxonomy.show_form(<?php echo $original_term->term_taxonomy_id ?>,'<?php echo $lang_code ?>');return false;">
                                ...
                                <form class="icl_tt_form hidden"
                                      id="icl_tt_form_<?php echo $original_term->term_taxonomy_id . '_' . $language['code'] ?>">
                                    <img src="<?php echo ICL_PLUGIN_URL . '/res/img/ajax-loader.gif' ?>" alt="loading"
                                         height="16" width="16" class="wpml_tt_spinner"/>
                                    <input type="hidden" name="translation_of"
                                           value="<?php echo $original_term->term_taxonomy_id ?>"/>
                                    <input type="hidden" name="taxonomy" value="<?php echo $this->taxonomy ?>"/>
                                    ...
                                    <table class="icl_tt_popup">
                                        ...
                                        <tr>
                                            ...
                                            <td><input name="name" type="text"
                                                       value="<?php echo isset($term->name) ? esc_attr($term->name) : '' ?>"/>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td><?php _e('Slug', 'sitepress') ?></td>
                                            <td><input name="slug" type="text"
                                                       value="<?php echo isset($term->slug) ? urldecode($term->slug) : '' ?>"/>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <td colspan="2" align="right">
                                                ...
                                                <input class="button-primary" type="submit"
                                                       value="<?php esc_attr_e('Ok', 'sitepress') ?>"/>
                                            </td>
                                        </tr>
                                    </table>
                                </form>
            ...
            <?php else: ?>
                <tr>
                    <td align="center"
                        colspan="<?php echo count($this->selected_languages) + 1 ?>"><?php _e('No terms found', 'sitepress') ?></td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
        ...
        <?php if ($this->trid_count > floor(WPML_TT_TERMS_PER_PAGE / count($this->selected_languages))): ?>
            <div class="tablenav bottom">
                <div class="tablenav-pages">
                    ...
                    <a class="first-page<?php if ($this->current_page == 1): ?> disabled<?php endif; ?>" href="#"
                       title="<?php esc_attr_e('Go to the first page', 'sitepress') ?>">&laquo;</a>
                    <a href="#" title="<?php esc_attr_e('Go to the previous page', 'sitepress') ?>"
                       class="prev-page<?php if ($this->current_page == 1): ?> disabled<?php endif; ?>">&lsaquo;</a>
                <span class="paging-input">
                    <input class="current-page" type="text" size="1" value="<?php echo $this->current_page ?>"
                           name="paged" title="<?php esc_attr_e('Current page', 'sitepress') ?>"/>
                    <?php _e('of', 'sitepress') ?>
                    <span
                        class="total-pages"><?php echo $total_pages = ceil($this->trid_count / WPML_TT_TERMS_PER_PAGE * count($this->selected_languages)) ?></span>
                </span>
                    <a class="next-page<?php if ($this->current_page == $total_pages): ?> disabled<?php endif; ?>"
                       href="#" title="<?php esc_attr_e('Go to the next page', 'sitepress') ?>">&rsaquo;</a>
                    <a class="last-page<?php if ($this->current_page == $total_pages): ?> disabled<?php endif; ?>"
                       href="#" title="<?php esc_attr_e('Go to the last page', 'sitepress') ?>">&raquo;</a>
                </div>
            </div>
        <?php endif; ?>
                    ...
                    <?php foreach ($this->taxonomy_obj->labels_translations as $language => $translations):
                        if ($language != $sitepress_settings['st']['strings_language']): ?>
                            <td>
                                ...
                                <a class="icl_tt_labels_<?php echo $this->taxonomy . '_' . $language ?><?php echo $class ?>"
                                   href="#"
                                ...
                                <form class="icl_tt_labels_form hidden"
                                      id="icl_tt_labels_form_<?php echo $this->taxonomy . '_' . $language ?>">
                                    <img src="<?php echo ICL_PLUGIN_URL . '/res/img/ajax-loader.gif' ?>" alt="loading"
                                         height="16" width="16" class="wpml_tt_spinner"/>
                                    <input type="hidden" name="taxonomy" value="<?php echo $this->taxonomy ?>"/>
                                    <input type="hidden" name="language" value="<?php echo $language ?>"/>
                                    <input type="hidden" name="singular_original"
                                           value="<?php echo $this->taxonomy_obj->labels_translations[$sitepress_settings['st']['strings_language']]['singular'] ?>"/>
                                    <input type="hidden" name="general_original"
                                           value="<?php echo $this->taxonomy_obj->labels_translations[$sitepress_settings['st']['strings_language']]['general'] ?>"/>
                                    <table class="icl_tt_popup">
                                        <tr>
                                            <td><?php _e('Singular', 'sitepress') ?></td>
                                            <td><input name="singular" type="text"
                                                       value="<?php echo $translations['singular'] ?>"/></td>
                                        </tr>
                                        <tr>
                                            <td><?php _e('Plural', 'sitepress') ?></td>
                                            <td><input name="general" type="text"
                                                       value="<?php echo $translations['general'] ?>"/></td>
                                        </tr>
                                        <tr>
                                            <td colspan="2" align="right">
                                                <span class="errors icl_error_text"></span>
            ...
            <form id="icl_tt_sync_assignment">
                ...
                <p>
                    <input class="button-secondary" type="submit"
                           value="<?php printf(__("Synchronize %s assignment in content", 'sitepress'), $this->taxonomy_obj->labels->name) ?>"/>
                    <img src="<?php echo ICL_PLUGIN_URL . '/res/img/ajax-loader.gif' ?>" alt="loading" height="16"
                         width="16" class="wpml_tt_spinner"/>
                </p>
                <span class="errors icl_error_text"></span>
            </form>
