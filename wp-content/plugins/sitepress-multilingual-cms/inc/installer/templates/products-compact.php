...
<div class="otgs_wp_installer_table otgs_wp_installer_table_compact">
    ...
    <?php if (!$this->repository_has_subscription($repository_id) || !empty($subscription_no_match)): ?>
        ...
        <a class="button-primary"
           href="<?php echo $this->append_parameters_to_buy_url($product['url'], $repository_id, $args) ?>"><?php printf(__('Buy %s', 'installer'), $args['product_name']) ?></a>
    ...
    <?php else: ?>
        ...
        <center>
            <a class="remove_site_key_js" href="#"
               data-repository=<?php echo $repository_id ?> data-confirmation="<?php esc_attr_e('Are you sure you want to remove this site key?', 'installer') ?>"
               data-nonce="<?php echo wp_create_nonce('remove_site_key_' . $repository_id) ?>"><?php printf(__("Remove current site key (%s)", 'installer'), $site_key) ?></a>
        </center>
        <br/>
    ...
    if (isset($args['name'])):
        ...
        <?php if ($support_link): ?>
        <p><a href="<?php echo $support_link ?>"
              target="_blank"><?php printf(__('%s support on wpml.org', 'installer'), $args['name']) ?></a></p>
