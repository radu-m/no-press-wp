...
class SitePressLanguageSwitcher
{
    ...
    function admin()
    {
        ...
        foreach ($this->color_schemes as $color_scheme_name => $color_scheme_data) {
            foreach ($this->widget_css_defaults as $scheme_attribute => $scheme_attribute_value) {
                ?>
                <input type="hidden"
                       id="icl_lang_sel_config_alt_<?php echo $color_scheme_name ?>_<?php echo $scheme_attribute ?>"
                       value="<?php echo $this->color_schemes[$color_scheme_name][$scheme_attribute] ?>"/>
                <?php
            }
        }
        ...
        <p>
            <label>
                <input type="checkbox" name="icl_lang_sel_footer" value="1"
                       <?php if (!empty($this->settings['icl_lang_sel_footer'])): ?>checked="checked"<?php endif ?> />
        ...
        if ($this->load_language_selector_css()) {
            ...
            <?php foreach ($this->color_schemes as $color_scheme_name => $color_scheme_data): ?>
                <?php foreach ($this->footer_css_defaults as $scheme_attribute => $scheme_attribute_value): ?>
                    <input type="hidden"
                           id="icl_lang_sel_footer_config_alt_<?php echo $color_scheme_name ?>_<?php echo $scheme_attribute ?>"
                           value="<?php echo $this->color_schemes[$color_scheme_name][$scheme_attribute] ?>"/>
                <?php endforeach; ?>
            <?php endforeach; ?>

            <p>
                <a href="#icl_lang_preview_config_footer_editor_wrapper" id="icl_lang_sel_footer_preview_link"
                   class="js-toggle-colors-edit">
            ...
            <div class="hidden" id="icl_lang_preview_config_footer_editor_wrapper">
                ...
                <span
                    style="display:none"><?php _e("Are you sure? The customization you may have made will be overridden once you click 'Apply'", 'sitepress') ?></span>
            ...
            <div class="wpml-section-content-inner">
                ...
                <ul>
                    ...
                    <li class="js-post-availability-settings <?php if (empty($this->settings['icl_post_availability'])): ?>hidden<?php endif ?>">
                        <label>
                            ...
                            <select name="icl_post_availability_position">
                                ...
                                <option
                                    value="below"<?php if (empty($this->settings['icl_post_availability_position']) || $this->settings['icl_post_availability_position'] == 'bellow'
                    ...
                    <li class="js-post-availability-settings <?php if (empty($this->settings['icl_post_availability'])): ?>hidden<?php endif ?>">
                        <label>
                            <?php _e('Text for alternative languages for posts', 'sitepress'); ?>: <input type="text"
                                                                                                          name="icl_post_availability_text"
                                                                                                          value="<?php
            ...
            <div class="wpml-section-content-inner">
                ...
                <p>
                    ...
                    ?>
                    <textarea id="icl_additional_css" name="icl_additional_css" rows="4"
                              class="large-text"><?php echo $icl_additional_css; ?></textarea>
                </p>
            </div> <!-- .wpml-section-content-inner -->
    ...
    function widget_list()
    {
        ...
        <div
            id="lang_sel_list"<?php if (empty($this->settings['icl_lang_sel_type']) || $this->settings['icl_lang_sel_type'] == 'dropdown') {
