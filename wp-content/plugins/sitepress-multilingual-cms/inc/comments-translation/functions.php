...
class IclCommentsTranslation
{
        ...
        */
        ...
        if (isset($_GET['retry_mtr'])) {
            ...
            if ($_GET['nonce'] == $nonce) {
                ...
                $delete_query_prepared = $wpdb->prepare($delete_query, $_GET['retry_mtr']);
    ...
    function comment_row_actions($actions, $comment)
    {
        ...
        ?>
        <script type="text/javascript">
            icl_comment_original_language.push({
                c:<?php echo $comment->comment_ID ?>,
                lang: '<?php echo $original_comment_language->language_code ?>',
                lang_name: '<?php echo $original_comment_language->name ?>'
            });
        </script>
        <div style="float:right;margin-top:4px;">
            <?php if ($this->user_language == $original_comment_language->language_code): ?>
                <small><a href="#c<?php echo $comment->comment_ID ?>"
                          class="icl_original_comment_link"><?php _e('Back to translated version', 'sitepress') ?></a>
                </small>
            <?php else: ?>
                <small><a href="#c<?php echo $comment->comment_ID ?>"
                          class="icl_original_comment_link"><?php printf(__('Original language: %s', 'sitepress'), $original_comment_language->name) ?></a>
                </small>
            <?php endif; ?>
        </div>
        <?php
    ...
    */
    ...
    function comment_form_options()
    {
        ...
        <label style="cursor:pointer">
            ...
            <input style="width:15px;" type="checkbox" name="icl_translate_reply"
                   checked="checked"<?php echo $disabled ?> />
        <span><?php echo sprintf(__('Translate from %s into %s', 'sitepress'), $user_lang_info['display_name'], $page_lang_info); ?>
            <?php if ($disabled): ?>
                <br/>
                <small><?php printf(__('There is no translator for this language pair. <a href="%s">Details</a>.', 'sitepress'), get_option('siteurl') . '/wp-admin/admin.php?page=' . basename(ICL_PLUGIN_PATH) . '/menu/content-translation.php'); ?></small>
            <?php endif; ?>
        </span>
        </label>
    <?php endif; ?>
            ...
            */
            ...
            if (!empty($trids)) {
                $res = $wpdb->get_results($wpdb->prepare("
                    ...
                    FROM {$wpdb->prefix}icl_translations
                    WHERE element_type='comment' AND trid IN (" . join(',', $trids) . ") AND language_code = %s
                ", $this->user_language));
                foreach ($res as $row) {
        ...
        if ($this->enable_comments_translation) {
            ...
            if (empty($comment_ids)) {
                ...
                $res_query = "
                    ...
                    WHERE t.element_id IN(" . join(',', $comment_ids) . ") AND t.element_type='comment' AND t.language_code <> %s AND (ms.object_type='comment' OR ms.object_type IS NULL)";
                ...
                if (!empty($tridsmap)) {
                    $res_query = "SELECT t.trid, c.comment_content
                        ...
                        WHERE t.trid IN(" . join(',', array_keys($tridsmap)) . ") AND t.element_type='comment' AND t.language_code=%s";
                    $res_query_prepared = $wpdb->prepare($res_query, $current_language);
                    $res = $wpdb->get_results($res_query_prepared);
                    foreach ($res as $row) {
